...
while (bmax - bmin) > 1:
    bnew = bmin + (bmax - bmin) // 2
    print(f"Testing {bnew}")
    t = Terrain(size)
    t.read_walls(lines, bnew)
    while t.extend_paths():
        pass
    if t.lost:
        bmax = bnew
        print(f"{bnew}: impassable")
    else:
        bmin = bnew
        print(f"{bnew}: passable")
t = Terrain(size)
t.read_walls(lines, bmax)
print(str(t), bmax)
print(f"{t.lastblock[0]},{t.lastblock[1]}")
