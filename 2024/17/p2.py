...
class Machine:
    ...
    def godeep(self, a, life):
        s = format(a, "0" + str(life) + "b") + f"({life})" if life > 0 else "(0)"
        print(f"GOING TO: {s} > ", end="")
        ...
        outoflife = False
        while running:
            # print(f"a={bin(a)} match={x} steps={self.steps}")
            test = self.run()
            if test == 0:  # FINISHED
                running = False
            if test == 1:  # NORMAL
            ...
            if test == 2:  # NO LIFE
                ...
                outoflife = True
        if mismatch:
            if len(self.program) < len(self.out):
                print(f"TOO LONG @{x-1} {self.out[x-1]} AT STEP {self.steps}")
            else:
                print(
                    f"MISMATCH @{x-1} {self.out[x-1]} instead of {self.program[x-1]} AT STEP {self.steps}"
                )
            return None
        if outoflife:
            print(f"OUT OF LIFE WITH {s} AT STEP {self.steps}")
            for i in range(4096):
                newa = i << life | a
                print(f"Forking from {s} with {format(i,'012b')}{s}")
                u = self.godeep(newa, life + 12)
                if u:
                    return u
            return None
        print("ENDED")
        print(self.out)
        print(self.program)
        print(running, mismatch, outoflife)
